...
export function placeCard(destino) {

    const { bg, text } = getCategoryStyle(destino.category);

    const session = getSession();
    const isExplorador = session?.user?.role === "explorador" || session?.role === "explorador";
    const placeId = destino.id ?? destino._id ?? "";

    const optionsButton = isExplorador
        ? `
            <button
                type="button"
                aria-label="Agregar a itinerario"
                data-place-id="${placeId}"
                data-place-name="${destino.place}"
                class="options-toggle-btn peer flex h-8 w-8 items-center justify-center rounded-full text-[#12293F] hover:scale-110 hover:bg-white border-0 outline-none ring-0 shadow-none cursor-pointer">

                ${iconSVG(ListPlus, { width: 18, height: 18, strokeWidth: 2 })}
